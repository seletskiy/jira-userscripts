...
try to reload a page or relogin to Jira.": "Ваша сессия истекла, пожалуйста, " +
	...
	var loadUi = function () {
		ui = {
			...
			opsbar: (function () {
				var ul = lib.$('<ul/>');
				ul.addClass('toolbar-group');
				ul.attr('id', 'opsbar-opsbar-transitions-start-stop');
			...
			worklogForm: (function () {
				...
				form.append(
					...
					'<div class="buttons submit">' +
						'<input id="worklog-submit" class="button submit_btn"' +
							'type="button" value="' + lib._("Log") + '"/>' +
						'<input id="worklog-submit-skip" class="button submit_btn"' +
	...
	var bindStopProgress = function () {
		...
		lib.$('#action_id_301').parents('li:first').remove().appendTo(ui.opsbar);
		ui.opsbar.append('<li class="worklog-dummy"/>');

		ui.opsbar.insertAfter('#opsbar-opsbar-operations');
		...
		ui.stopProgressButton.attr('href', 'javascript:void(null)');
		...
		ui.startProgressButton.click(function () {
			...
			var refreshLoadState = function () {
				...
				if (checkLoaded()) {
					...
					setTimeout(refreshLoadState, 1000);
				}
			}
	...
	var styles = {
		...
		"#stalker #worklog-form .time-spent": [
			"margin-bottom: 5px",
			"line-height: 23px"
		],
		...
		"#stalker #worklog-form .buttons": [
			"margin-top: 5px"
		],
		"#stalker #worklog-form .button": [
			"margin-right: 10px"
		]
	};
